...
if (!entry) {
  ...
  if (typedEntry.sms_opt_in === true && typedEntry.contact_phone) {
    try {
      ...
      const summary = await sendMissionControlSms({
        organizationId,
        eventId,
        channel: "parade_units",
      ...
      smsSent = summary.sent > 0;
      if (summary.failed > 0 || summary.attempted === 0) {
